...
# # === Helper: Build FAISS index from chunks ===
...
#     selected = [chunks[i]['text'] for i in I[0]]
#     context = "\n\n".join(selected)
#     prompt = f"Context:\n{context}\n\nQuestion: {question}\nAnswer:"
#     response = ollama.chat(
#         model="qwen3:8b",  # or your preferred local Ollama model
...
def rag_answer(chunks, question, index=None, embeddings=None):
    ...
    response = ollama.chat(
        model="phi3",  # or your preferred Ollama model
        messages=[{"role": "user", "content": prompt}]
    )
    return response['message']['content'].strip()
